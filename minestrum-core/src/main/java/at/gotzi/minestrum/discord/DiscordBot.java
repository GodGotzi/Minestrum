    ...
    public DiscordBot start() {
        ...
        try {
            ...
            while (!atomicBoolean.get()) {
                this.logger.info("Waiting for Discord Bot...");
                Thread.sleep(200);
            }

            this.logger.info("Discord-Bot is ready to hear your commands!");

        } catch (Exception e) {
            Minestrum.getInstance().getLogger().warning("Could not build Discord Bot -> " + e.getMessage());
            return null;
        }
